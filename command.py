...
import data_driven_force_calculator
import glob
...
def get_values():
	...
	force_prediction = data_driven_force_calculator.force_calc()
	...
	while x_error > 0.05 or y_error > 0.05:
		...
		force_prediction = data_driven_force_calculator.force_calc(polar_rad, polar_angle)
		...
		if force_prediction < max_force:
			...
			if (np.abs(next_x_rel) < np.abs(x_goal)):
				...
				while force_prediction > max_force:
					...
					print ""
					force = data_driven_force_calculator.beam_adjust()
